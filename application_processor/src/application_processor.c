...
int validate_components() {
    ...
    for (unsigned i = 0; i < flash_status.component_cnt; i++) {
        ...
        int result_s = send_packet(addr, sizeof(uint8_t), transmit_buffer);
        if (result_s == ERROR_RETURN) {
            print_error("Could not validate component\n");
            return ERROR_RETURN;
        }
        //int len = issue_cmd(addr, transmit_buffer, receive_buffer);
        /**********************************************************************/
        int result_r = poll_and_receive_packet(addr, receive_buffer);
        if (result_r == ERROR_RETURN) {
            print_error("Could not validate component\n");
            return ERROR_RETURN;
        }
        memcpy((void *)receive_sign, receive_buffer, 64);
        memcpy((void *)receive_hash, receive_buffer + 64, 32);
        int verify_result = verify_key(Comp_X, Comp_Y, receive_hash, receive_sign, curve_secp);
        if (verify_result == ERROR_RETURN) {
            print_error("Could not validate component\n");
            return ERROR_RETURN;
        }
        uint8_t * hash = generate_nonce();
        uint8_t * sign = sign_key(AP_Private_Key, hash, curve_secp);
        if (sign == NULL) {
        ...
        int send = send_packet(addr, sizeof(uint8_t) * 96, package);
        if (send == ERROR_RETURN) {
            print_error("Could not validate component\n");
            return ERROR_RETURN;
        }
        free(sign);
...
int attest_component(uint32_t component_id) {
    ...
    uint8_t rng_nonce0[16];
    RNG_init(rng_nonce0, sizeof(rng_nonce0));
    uECC_set_rng(&RNG_init);
    const struct uECC_Curve_t * curve_secp = uECC_secp256k1();
    // Create command message
    command_message* command = (command_message*) transmit_buffer;
